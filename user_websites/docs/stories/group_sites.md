...
- **When** I click "Create Site", the system initializes the home page for the group ([@ANCHOR: UX_CREATE_SITE]).
...
- **Then** they can create new blog posts ([@ANCHOR: UX_CREATE_BLOG_POST]).
- **And** all posts are attributed to the group's shared blog container.

## Technical Notes
- Group site routing follows similar logic to personal sites ([@ANCHOR: controller_user_websites_home]).
- Access to edit or delete group pages is restricted to members of the associated Odoo group ([@ANCHOR: mixin_proxy_ownership_write]).
- Slugs for groups are also cached in Redis ([@ANCHOR: group_slug_cache_invalidation]).
- Group members can be unsubscribed from notifications using a secure link ([@ANCHOR: controller_unsubscribe_digest]).

...
class TestKeyRegistry(TransactionCase):
    ...
    def test_register_daemon_api(self):
        ...
        result = self.env["daemon.key.registry"].register_daemon(
            daemon_name, user_xml_id, env_file_path
        )
    ...
    def test_documentation_installed(self):
        ...
        if model:
            ...
            self.env["ir.module.module"].with_user(
                self.env.ref("zero_sudo.odoo_facility_service_internal").id
            )._bootstrap_knowledge_docs()

            article = self.env[model].search(
    ...
    def test_key_ownership(self):
        ...
        self.assertNotEqual(
            res[0], SUPERUSER_ID, "Key should not be owned by SUPERUSER"
        )
...
class TestKeyRegistryTour(HttpCase):
    def test_daemon_key_manager_tour(self):
        ...
        self.start_tour(
            "/odoo?action=daemon_key_manager.action_daemon_key_registry",

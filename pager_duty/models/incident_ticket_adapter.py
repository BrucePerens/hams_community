...
class PagerDutyIncidentTicketAdapter(models.Model):
    ...
    def action_generate_helpdesk_ticket(self):
        ...
        for incident in self:
            ...
            try:
                ...
                _logger.warning(
                    "Failed to retrieve system param for helpdesk model: %s", e
                )
            ...
            try:
                ...
                try:
                    ...
                    _logger.warning(
                        "Failed to resolve hams_helpdesk service account: %s", e
                    )
                ...
                if assignee_id:
                    ...
                    if user.partner_id:
                        try:
                            ...
                            _logger.warning(
                                "Failed to resolve pager_duty service account for calendar event: %s",
                                e,
                            )
                ...
                _logger.warning(
                    "Permission or Validation error during ticket creation for %s: %s",
                    incident.name,
                    e,
                )
                ...
                _logger.error(
                    "Unexpected Helpdesk Ticket Creation Failed for Incident %s: %s",
                    incident.name,
                    e,
                )
                self._execute_smtp_fallback(incident, str(e), assignee_id)
    ...
    def _execute_smtp_fallback(self, incident, error_msg, assignee_id=False):
        ...
        try:
            ...
            _logger.warning(
                "Failed to resolve pager_duty service account for fallback: %s", e
            )
            pd_uid = self.env.user.id

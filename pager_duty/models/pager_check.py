...
class PagerCheck(models.Model):
    ...
    status = fields.Selection([('passing', 'Passing'), ('failing', 'Failing'), ('maintenance', 'Maintenance')], string="Status", default='passing', readonly=True)
    ...
    grace_period = fields.Integer(string="Startup Grace Period (sec)", default=0, help="Seconds to wait after daemon startup before reporting failures.")
    active = fields.Boolean(default=True)

    parent_check_id = fields.Many2one('pager.check', string="Parent Check (Dependency)", help="If parent fails, this check is suppressed.")
    maintenance_start = fields.Datetime(string="Maintenance Start")
    maintenance_end = fields.Datetime(string="Maintenance End")
    auto_remediate_script = fields.Char(string="Auto-Remediation Script", help="Executed locally by daemon if check fails (e.g., /opt/reboot.sh)")

    heartbeat_uuid = fields.Char(string="Heartbeat UUID", default=lambda self: str(uuid.uuid4()), readonly=True)
    ...
    def action_trigger_check(self):
        return {
            ...
            'params': {
                ...
                'message': _('The external SRE daemon operates asynchronously outside of Odoo. It will execute this check on its next polling cycle.'),
                'type': 'info',
                'sticky': False,
            }
        }

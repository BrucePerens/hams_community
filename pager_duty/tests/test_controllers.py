...
class TestPagerControllers(HttpCase):
    def test_01_ping_endpoint(self):
        ...
        self.assertEqual(response.status_code, 200, 'Ping endpoint failed to return 200 OK.')
        self.assertIn('"status": "ok"', response.text, 'Ping endpoint returned invalid JSON payload.')
    ...
    def test_02_board_security_and_render(self):
        ...
        self.assertTrue('web/login' in response.url, 'Board endpoint failed to enforce auth="user" security mandate.')

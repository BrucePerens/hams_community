...
class TestPagerSchedule(TransactionCase):
    ...
    def test_01_get_current_on_duty_admin_active(self):
        ...
        self.assertEqual(on_duty.id, self.test_user.id, 'Scheduling engine failed to return the currently active admin.')
    ...
    def test_02_get_current_on_duty_admin_expired(self):
        ...
        on_duty = self.calendar_model.get_current_on_duty_admin()
        self.assertFalse(on_duty, 'Scheduling engine returned an admin for an expired shift.')
    ...
    def test_03_get_current_on_duty_admin_non_pager(self):
        ...
        on_duty = self.calendar_model.get_current_on_duty_admin()
        self.assertFalse(on_duty, 'Scheduling engine returned an admin for a non-pager-duty calendar event.')

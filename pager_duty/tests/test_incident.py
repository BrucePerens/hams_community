...
class TestPagerIncident(TransactionCase):
    ...
    def test_03_bus_notification_on_create(self):
        if self.integration_mode:
            ...
            with patch.object(type(self.env['bus.bus']), '_sendone') as mock_sendone:
                ...
                self.assertEqual(str_args[0], 'pager_duty', 'Bus notification sent to incorrect channel.')
                self.assertEqual(str_args[1], 'update_board', 'Bus notification used incorrect message type.')

    def test_05_mtta_mttr_calculation(self):
        # Prove MTTA/MTTR computation
        incident = self.incident_model.create({'source': 'analytics_test', 'severity': 'low', 'description': 'desc'})
    ...
    def test_06_escalation(self):
        ...
        incident = self.incident_model.create({'source': 'esc_test', 'severity': 'high', 'description': 'desc'})
        import datetime

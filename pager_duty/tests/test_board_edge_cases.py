...
class TestBoardEdgeCases(TransactionCase):
    def setUp(self):
        ...
        self.user = self.env['res.users'].create({
            ...
            'group_ids': [(6, 0, [self.env.ref('pager_duty.group_pager_admin').id, self.env.ref('base.group_user').id])]
        })
    ...
    def test_02_acknowledge_rpc(self):
        ...
        incident = self.env['pager.incident'].create({'source': 'test', 'severity': 'high', 'description': 'desc'})

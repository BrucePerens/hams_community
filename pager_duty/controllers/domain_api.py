...
class PagerDutyController(http.Controller):
    ...
    @http.route('/api/v1/pager_duty/update_domains', type='jsonrpc', auth='public', methods=['POST'], csrf=False)
    def update_domains(self, **kwargs):
        ...
        request.env['pager.check'].with_user(svc_uid).update_lets_encrypt_domains(domains)

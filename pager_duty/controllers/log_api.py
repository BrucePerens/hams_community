...
class PagerLogAPI(http.Controller):
    ...
    def search_logs(self, file_path, regex_query):
        ...
        if not request.env.user.has_group("pager_duty.group_pager_admin"):
            raise AccessError("Access Denied: Admins only.")

        if not redis or not redis_pool:
            return {"error": "Redis not available for IPC."}
        ...
        try:
            ...
            return {"error": "Search timeout. Daemon may be offline."}
        except Exception as e:
            return {"error": f"IPC Failure: {e}"}
    ...
    def get_log_files(self):
        ...
        files = request.env["pager.log.file"].with_user(svc_uid).search([("active", "=", True)]).mapped("filepath")
        return {"files": files}

...
class PagerConfigWizard(models.TransientModel):
    ...
    yaml_content = fields.Text(string="YAML Configuration", default=lambda self: self._default_yaml())

    def _get_config_path(self):
        # Securely resolve the absolute path to the daemon's configuration file in the repository
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'daemons', 'pager_duty'))
    ...
    def action_load_from_file(self):
        ...
        return {"type": "ir.actions.act_window", "res_model": "pager.config.wizard", "res_id": self.id, "view_mode": "form", "target": "new"}

    def action_generate_yaml(self):
        """Reads the database records and generates a fresh YAML representation."""
        if not yaml:
            raise UserError(_("PyYAML is not installed. Please run pip install PyYAML."))
            
        checks = self.env['pager.check'].search([('active', '=', True)], limit=1000)
        check_list = []
        for c in checks:
            d = {'name': c.name, 'type': c.check_type, 'target': c.target, 'interval': c.interval}
    ...
    def action_save_to_file_and_db(self):
        ...
        for c in checks:
            if c.get('parent') and c.get('parent') in name_to_id and c.get('name') in name_to_id:
                check_rec = next((r for r in all_checks if r.id == name_to_id[c.get('name')]), None)
                if check_rec:

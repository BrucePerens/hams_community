...
class TestZeroSudoControllers(HttpCase):
    ...
    def test_01_web_login_interceptor(self):
        ...
        response = self.url_open('/web/login') # burn-ignore-route  # fmt: skip
        csrf_token = ""
        ...
        response = self.url_open('/web/login', data={ # burn-ignore-route  # fmt: skip
            'login': login,
            'password': password,
        ...
        self.assertIn('/web/login', response.url) # burn-ignore-route  # fmt: skip

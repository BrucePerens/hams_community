...
class CloudflareRoutingDomain(models.Model):
    ...
    cloudflare_hostname_id = fields.Char("Cloudflare Hostname ID", readonly=True, help="ID returned by Cloudflare for management")
    ssl_status = fields.Selection([
        ...
        ("active", "Active"),
        ("error", "Error")
    ], string="SSL Status", default="pending_validation", readonly=True)
    ...
    def _create_cloudflare_custom_hostname(self):
        self.ensure_one()
        cf_utils = self.env['cloudflare.utils'].with_user(self.env.ref('base.user_admin'))
        ...
        if token and zone_id:
            ...
            if success and isinstance(result, dict):
                ...
                self.ssl_status = result.get('ssl', {}).get('status', 'pending_validation')
    ...
    def action_sync_ssl_status(self):
        for record in self:
            ...
            if token and zone_id:
                success, result = cf_utils.get_custom_hostname(record.cloudflare_hostname_id, token, zone_id)
                if success and isinstance(result, dict):

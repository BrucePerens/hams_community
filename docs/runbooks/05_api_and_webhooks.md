...
We use HMAC-SHA256 signatures so headless clients can safely bypass Odoo's native CSRF protection.
...
* **Trigger:** When Odoo commits a batch of QSOs, it fires a `NOTIFY` (`[@ANCHOR: qso_firehose_notify]`) containing only the comma-separated integer IDs of the new records.

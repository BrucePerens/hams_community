...
We provide several REST endpoints for ingesting data and rendering the frontend.
Our secured endpoints use HMAC-SHA256 signatures to safely bypass standard CSRF protections.
*Architectural Security Note:* We protect all automated REST API endpoints against RPC Mass Assignment vulnerabilities.
They run using the **Service Account Pattern** (e.g., `user_logbook_api_service`), which strictly binds operations to least-privilege Record Rules.
...
* **Batch ADIF Download:** `GET /api/v1/ham_logbook/adif/download`
  ...
  * **Purpose:** Allows external services to download a user's logbook. Supports `limit`, `offset`, `start_date`, and `end_date` query parameters.
Protected by a strict Nginx rate limit.
* **Real-Time QSO Logging:** `POST /api/v1/ham_logbook/qso/live`
  * **Auth:** Required (`X-Odoo-User`, `X-Odoo-Signature`).
  * **Purpose:** Instantly commits a single JSON-formatted QSO to the database.
Designed for use with local desktop relays listening to WSJT-X / N1MM broadcasts.
...
* **Callsign Lookup:** `GET /api/v1/ham_shack/lookup/{callsign}`
  * **Auth:** Required (`user`).
  * **Purpose:** Queries `ham_callbook` and `ham_logbook` for regulatory data and 4-hour historical context.
Returns a JSON payload for Fast Entry auto-fill.
...
### F. Identity & Onboarding (`ham_onboarding`)
* **QRZ Verification Status:** `GET /api/v1/onboarding/qrz/status`
  * **Auth:** Required (`user`).
  * **Purpose:** A lightweight polling endpoint that triggers a background scrape of the user's QRZ.com biography.
Returns `{"status": "pending"}` or `{"status": "verified"}`.

### G. Pager Duty & SRE (`pager_duty`)
* **Heartbeat Ping:** `POST /api/v1/pager/heartbeat/<string:hb_uuid>` (Also accepts `GET`)
  * **Auth:** Public (Security through UUID obscurity).
  * **Purpose:** "Dead Man's Snitch" push monitoring endpoint. External scripts ping this to prove they ran successfully. Updates the `last_heartbeat` timestamp on the target monitoring check.
...
* **Purpose:** A high-throughput, async Python daemon bypassing Odoo web workers entirely.
It broadcasts a JSON feed of every QSO, POTA/SOTA activation, and contest exchange logged globally on the platform the millisecond it occurs.
...
1.  **API Secret (HMAC-SHA256 for POST):** Used for REST data ingestion.
The client computes an HMAC hash of the concatenated `timestamp|nonce|payload` using their `adif_api_secret` to prevent replay attacks.
2.  **API Secret (HMAC-SHA256 for GET):** Used for REST data retrieval.
The client computes an HMAC hash of the concatenated `timestamp|nonce`.
The server enforces a strict 5-minute TTL on the timestamp and validates the Nonce against Redis.
...
4.  **Client mTLS (ARRL LoTW):** Used strictly during the `ham_onboarding` flow.
Nginx intercepts the `.p12` cryptographic certificate and passes the DN headers to Odoo to verify identity.

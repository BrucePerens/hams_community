...
*Exception:%2* The `mail_service_internal` account retains `base.group_user` strictly because the Odoo `mail` module deeply requires it to differentiate between internal system notes (`mail.mt_note`) and portal customer messages.

## Consequences
Daemons and Service Accounts remain absolutely isolated to their designated tables. A compromised Callbook daemon cannot read internal financial accounts, messages, or HR data.

...
## Context
Controllers, widgets, and APIs frequently need to present aggregated or masked data (e.g., POTA hunt counts, fuzzed geographic locations, or active repeater lists) to public guests or unprivileged standard users. 

Previously, to fetch this data, the system escalated privileges by invoking a Micro-Service Account (e.g., `with_user(svc_uid)`) to query the restricted root tables (`ham.qso`, `ham.callbook`). The Python controller would then manually count the records or mask the sensitive fields (like exact street addresses) before returning the JSON response. 
...
1.  **Shift-Left Masking:** The SQL `CREATE VIEW` statement is responsible for stripping PII, truncating grid squares, or counting rows.

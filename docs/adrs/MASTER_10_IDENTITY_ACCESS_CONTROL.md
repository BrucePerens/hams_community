...
* Users manage personal websites and blogs. Because Odoo restricts core UI creation (`ir.ui.view`) to administrators, models MUST inherit the `user_websites.owned.mixin` (or equivalent proxy mixin).
...
* The master database password MUST NEVER be stored in plaintext. It must be pre-hashed (PBKDF2-SHA512) via the appropriate setup utility. The core initialization module uses raw SQL to inject it, bypassing Odoo's double-hashing ORM.
...
* All community users must be restricted to `base.group_portal` combined with domain-specific groups (e.g., `user_websites.group_user_websites_user`).
...
* If a user requires elevated rights to interact with a native Odoo model (such as submitting a generic survey or application), the transaction MUST be proxied through a Service Account using the Zero-Sudo architecture.

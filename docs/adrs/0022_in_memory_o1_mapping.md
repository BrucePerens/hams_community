...
4. **Batch Commit:** Commit updates using bulk `.write()` or `create()` operations at the end of each chunk's lifecycle, rather than one record at a time. To prevent transaction lock exhaustion, you MUST explicitly call `self.env.cr.commit()` inside the `while True` loop after processing each chunk to release PostgreSQL locks before sleeping.

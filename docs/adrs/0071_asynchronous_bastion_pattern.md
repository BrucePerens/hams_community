...
1. **State Initialization:** The Odoo controller or ORM method creates a tracking record (e.g., `ham.dns.zone` or `ham.adif.queue`) and sets its status field to `pending`.

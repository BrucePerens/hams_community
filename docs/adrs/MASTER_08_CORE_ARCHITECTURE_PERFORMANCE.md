...
### 7. Transaction Boundary Bypassing (Subprocess Execution)
* Certain PostgreSQL maintenance commands (like `VACUUM ANALYZE` or `pgBackRest` executions) cannot be executed inside Odoo's standard ORM transaction blocks without triggering `ActiveSqlTransaction` crashes.
* To execute these operations, the system MUST orchestrate the native OS-level binaries (e.g., `vacuumdb`) via Python's `subprocess.run` module.
* To prevent shell injection, these executions MUST enforce `shell=False` and pass arguments as strict arrays rather than concatenated strings.

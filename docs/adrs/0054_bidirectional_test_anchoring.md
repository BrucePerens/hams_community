...
1. **Source Code:** Near a semantic anchor in the execution logic, developers MUST add a comment pointing to the test anchor(s) verifying it. Example: `# Verified by [%ANCHOR: test_qso_cross_match]`.
2. **Test Code:** In the test method, developers MUST add an anchor for the test itself AND a comment referencing the source code anchor it verifies. Example: `# Tests [%ANCHOR: qso_cross_match]`.
3. **Frontend Web Tours (JavaScript):** User-side JS interactions (OWL components or HTML UI flows) MUST utilize bidirectional anchoring between the UI source and the Web Tour. Example (JS): `// Verified by [%ANCHOR: test_tour_my_feature]` -> `// Tests [%ANCHOR: my_feature]`.

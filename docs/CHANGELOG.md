...
- **Administrative Impersonation**: Added a secure 'Login As' facility to `ham_base` allowing admins to temporarily assume a user's session for troubleshooting, strictly gated by an immutable Chatter audit log.
### Changed
...
- **Separation of Privilege**: Eliminated the final authorized uses of `.sudo().unlink()` and `base.user_admin` escalations for GDPR operations and Club Governance voting by introducing dedicated micro-accounts (`gdpr_service_internal` and `club_voter_service_internal`), further hardening the Zero-Sudo architecture.
...
### Fixed
- **Pager Duty UI Stability**: Fixed XML ParseErrors caused by mismatched button actions (`action_save_to_file_and_db`) and missing model fields (`status`, `last_run`, `payload_expect`) in the monitoring and configuration views.
- **Impersonation Audit Trail**: Fixed an issue where the 'Login As' action attempted to post chatter messages directly to `res.users` instead of the underlying `res.partner`, causing application crashes.
- **Test Suite Resiliency**: Resolved false-negative test failures by correctly scoping global `subprocess` mocks and utilizing `mute_logger` to suppress expected Werkzeug 403 AccessError tracebacks during HTTP impersonation testing.
- **Parcel Extractor Permissions**: Fixed an issue in `tools/parcel_extract.py` where atomic write operations stripped executable file permissions by ensuring `shutil.copymode` is applied.

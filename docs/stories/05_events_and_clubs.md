...
* **Story:** As a Net Control Station (NCS), I want to see a dynamically updating list of participants checking into the frequency, including their specific traffic flags, to maintain orderly communications. *(Reference: ham_events/static/src/js/net_roster_widget.js -> handleCheckin -> [@ANCHOR: UX_LIVE_NET_ROSTER])*
    * **BDD Criteria:**
...
* **Story:** As a community participant, I want to flag inaccuracies on an event listing, dispatching an automated alert directly to the organizers to ensure the calendar remains reliable. *(Reference: ham_events/controllers/main.py -> report_event_issue -> [@ANCHOR: UX_REPORT_EVENT_ISSUE])*
    * **BDD Criteria:**
...
* **Story:** As a club member, I want a centralized dashboard to view active board elections and my current membership status, so I understand my eligibility to participate in governance. *(Reference: ham_club_management/static/src/xml/club_dashboard.xml -> ClubDashboard -> [@ANCHOR: UX_CLUB_GOVERNANCE_UI])*
    * **BDD Criteria:**

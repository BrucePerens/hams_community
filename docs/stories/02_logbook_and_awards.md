...
* **Story:** As a user seeking confirmation, I want to trigger an automated dispatch that politely emails the remote station, encouraging them to synchronize their records with the platform. *(Reference: ham_logbook/models/ham_qso.py -> action_nudge_station -> [@ANCHOR: UX_QSO_NUDGE_STATION])*
    * **BDD Criteria:**
...
* **Story:** As a human operator, I want to drag my ADIF file into the browser and watch it process. *(Reference: ham_logbook/controllers/website_logbook.py -> web_adif_upload -> [@ANCHOR: UX_HUMAN_ADIF_UPLOADER])*
    * **BDD Criteria:**

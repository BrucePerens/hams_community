...
* **Story:** As a trusted operator, I want a clean, accessible interface to browse and post equipment listings, with clear visual indicators of my verification status. *(Reference: ham_classifieds/static/src/xml/classifieds_board.xml -> ClassifiedsBoard -> [@ANCHOR: UX_CLASSIFIEDS_UI_GRID])*
    * **BDD Criteria:**
...
* **Story:** As a verified user, I want a clean HTML form to submit new classified listings directly from the website.
    * **BDD Criteria:**
        * *Given* an authenticated verified user
        * *When* they access the new listing page
        * *Then* the form MUST render securely. *(Reference: [@ANCHOR: UX_CLASSIFIEDS_HTML_FORM])*
...
* **Story:** As a zone owner, I want a frontend form to easily add new A, CNAME, or TXT records to my DNS zone.
    * **BDD Criteria:**
        * *Given* a user managing their DNS zone
        * *When* they use the frontend portal
        * *Then* the form MUST securely add the record to their specific zone. *(Reference: [@ANCHOR: UX_FRONTEND_DNS_RECORD_ADD])*

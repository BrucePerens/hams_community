...
* **Story:** As a prospective USA-based licensed user, I want to answer a technical radio question from the FCC examination pool instead of solving image puzzles during signup, so that I can prove I possess domain knowledge. *(Reference: ham_testing/models/ham_captcha.py -> generate_challenge -> [@ANCHOR: generate_ham_captcha])*
    * **BDD Criteria:** * *Given* an unauthenticated session on the signup page selecting the 'Licensed Operator' path
...
* **Story:** As a security architect, I want the challenge system to utilize cryptographically secure, time-limited tokens, so that automated scripts cannot recycle previous answers to bypass the gateway. *(Reference: ham_testing/models/ham_captcha.py -> verify_answer -> [@ANCHOR: verify_ham_captcha])*
    * **BDD Criteria:** * *Given* an active CAPTCHA session older than 15 minutes
...
* **Story:** As a platform defender, I want public signup forms to include a hidden honeypot field to silently drop automated bots without annoying legitimate users.
    * **BDD Criteria:**
        * *Given* an automated bot filling all fields including the hidden honeypot
        * *When* the signup form is submitted
        * *Then* the system MUST silently ignore the registration, log the bot IP, and redirect to a fake success page. *(Reference: [@ANCHOR: signup_honeypot])*
...
* **Story:** As a licensed operator proficient in CW, I want an optional challenge to verify my identity by manually transmitting my callsign using the spacebar, adapting to my natural speed between 5 and 40 WPM. *(Reference: ham_onboarding/static/src/js/morse_challenge.js -> _decode -> [@ANCHOR: UX_PROCESS_MORSE_INPUT])*
    * **BDD Criteria:**
        * *Given* a user operating the Morse challenge widget
        * *When* they key their exact callsign at any relative speed
        * *Then* the JS clustering algorithm MUST correctly decode the input and the backend MUST transition their identity state to verified. *(Reference: [@ANCHOR: verify_morse_challenge])*
...
* **Story:** As a user verifying via email, I want an OTP sent to the official address. *(Reference: ham_onboarding/models/res_users_verification.py -> action_send_official_otp -> [@ANCHOR: UX_GENERATE_OFFICIAL_OTP])*
    * **BDD Criteria:**
        * *Given* an unverified user requesting email verification
        * *When* the action executes
        * *Then* an OTP must be sent to their regulatory email.
* **Story:** As a user verifying via QRZ, I want the system to poll my bio. *(Reference: ham_onboarding/models/res_users_verification.py -> action_verify_qrz_bio -> [@ANCHOR: UX_VERIFY_QRZ_BIO])*
    * **BDD Criteria:**
        * *Given* an unverified user requesting QRZ verification
        * *When* the background queue executes
...
* **Story:** As an active operator, I want to present my digital `.p12` file to authenticate, so that I can bypass standard login forms and instantly gain trusted status. *(Reference: ham_onboarding/controllers/lotw_auth.py -> consume_lotw_token -> [@ANCHOR: lotw_token_consumption])*
    * **BDD Criteria:** * *Given* a valid `lotw_verified_callsign` payload
...
## Epic: Administrative Support & Impersonation
* **Story:** As a System Administrator, I want to temporarily impersonate a user's session without their password, so I can troubleshoot localized frontend bugs or missing awards.
    * **BDD Criteria:**
        * *Given* a system administrator viewing a user profile
        * *When* they trigger the "Login As" action
        * *Then* the system MUST swap the session UID and instantly post an immutable audit log to the user's chatter history. *(Reference: [@ANCHOR: action_impersonate_user])*
* **Story:** As a security auditor, I want mathematical proof that the impersonation feature cannot be abused silently.
    * **BDD Criteria:**
        * *Given* the automated test suite
        * *When* the impersonation action is tested
        * *Then* the test MUST assert that the `mail.message` was successfully generated on the target partner record. *(Reference: [@ANCHOR: test_impersonation])*

...
* **Linter Dodging / Variable Assignment:** You are STRICTLY FORBIDDEN from decoupling dangerous calls or using semantic tricks to bypass linter checks. 
  * Do not use boolean obfuscation like `csrf=not True` to bypass `csrf=False` checks.
  * Do not decouple string literals like `'.su' + 'do()'` or `'<' + '!--'`.
  * Do not assign `.sudo()` to an intermediate variable (e.g., `Model = request.env['...'].sudo()`).
  * Do not build f-strings mapped to intermediate variables and then pass them into `cr.execute()`.
  The linter enforces architectural mandates; contorting code to silently bypass it is a severe violation. If an exception is legally warranted, update the `EXEMPTIONS` dictionary in `check_burn_list.py` instead of obfuscating the code.
...
* **The `# burn-ignore` Prohibition:** You are STRICTLY FORBIDDEN from unilaterally appending `# burn-ignore` to bypass linter checks out of convenience or to resolve your own generated errors. It is reserved EXCLUSIVELY for the specific cryptographic fetching of `database.secret` within the centralized security utility. If your code triggers a Burn List error, you MUST rewrite your code using modern Odoo native idioms to pass the check naturally.
...
* **The "Centralized Security Utility" Pattern:**
    ...
    * **Skeleton Key Prevention (RPC & SSTI):**
        ...
        * If a controller strictly requires a cryptographic secret (e.g., for HMAC signing), it must bypass the utility and use `.sudo().get_param('database.secret')` inline, appending `# burn-ignore` to the line to explicitly declare the security exception to the linter.

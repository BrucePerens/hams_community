...
class HamBackupSnapshot(models.Model):
    ...
    config_id = fields.Many2one('backup.config', string="Configuration", required=True, ondelete='cascade')
    ...
    def _compute_restore_command(self):
        ...
        for rec in self:
            if rec.config_id.engine == 'kopia':
                rec.restore_command = f"kopia restore {rec.snapshot_id} /tmp/restore_dir"

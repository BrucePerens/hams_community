...
class BackupConfig(models.Model):
    ...
    _sql_constraints = [
        ('_name_uniq', 'UNIQUE(name)', 'The backup configuration name must be unique!'),
        ('_target_uniq', 'UNIQUE(engine, target_path)', 'The target path must be unique per engine!'),
        ('_name_not_empty', 'CHECK(LENGTH(TRIM(name)) > 0)', 'The configuration name cannot be empty.'),
        ('_target_path_not_empty', 'CHECK(LENGTH(TRIM(target_path)) > 0)', 'The target path cannot be empty.'),
        ('_retention_positive', 'CHECK(keep_daily >= 0 AND keep_weekly >= 0 AND keep_monthly >= 0)', 'Retention values cannot be negative.'),
        ('_min_size_positive', 'CHECK(minimum_size_mb >= 0)', 'Minimum size threshold cannot be negative.'),
    ]

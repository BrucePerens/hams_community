...
class HamBackupConfig(models.Model):
    ...
    storage_type = fields.Selection([('local', 'Local Directory'), ('s3', 'AWS S3'), ('b2', 'Backblaze B2')], default='local', string="Storage Type")
    ...
    secret_key = fields.Char(string="Secret Key", compute="_compute_secret_key", inverse="_inverse_secret_key")
    ...
    exclude_patterns = fields.Text(string="Exclude Patterns (.kopiaignore)", help="One pattern per line.")

    restore_drill_script = fields.Char(string="Automated Restore Drill Script", help="Absolute path to a shell script that performs a test restore and data validation.")
    ...
    def _compute_kopia_password(self):
        ...
        for rec in self:
            if rec.kopia_password_crypt and f:
                try:
                    rec.kopia_password = f.decrypt(rec.kopia_password_crypt.encode('utf-8')).decode('utf-8')
                except Exception:
    ...
    def _inverse_kopia_password(self):
        ...
        for rec in self:
            if rec.kopia_password and f:
                rec.kopia_password_crypt = f.encrypt(rec.kopia_password.encode('utf-8')).decode('utf-8')
            else:
    ...
    def _compute_secret_key(self):
        ...
        for rec in self:
            if rec.secret_key_crypt and f:
                try:
                    rec.secret_key = f.decrypt(rec.secret_key_crypt.encode('utf-8')).decode('utf-8')
                except Exception:
    ...
    def _inverse_secret_key(self):
        ...
        for rec in self:
            if rec.secret_key and f:
                rec.secret_key_crypt = f.encrypt(rec.secret_key.encode('utf-8')).decode('utf-8')
            else:
    ...
    def _get_executable(self, engine):
        ...
        if engine == 'kopia':
            ...
            try:
                ...
                os.unlink(tmp.name)
                self.message_post(body=_("Kopia successfully installed to %s") % kopia_bin) # audit-ignore-mail: Tested by [%ANCHOR: test_backup_orchestration]
                return kopia_bin
    ...
    def _trigger_kopia_backup(self):
        try:
            ...
            if self.kopia_password: env_vars['KOPIA_PASSWORD'] = self.kopia_password
            res = subprocess.run([exe, 'snapshot', 'create', self.target_path, '--json'], capture_output=True, text=True, timeout=3600, env=env_vars, shell=False)
            if res.returncode != 0:
                self._report_backup_failure(f"Kopia backup failed: {res.stderr}")
            else:
                self.message_post(body=_("Kopia backup completed successfully.")) # audit-ignore-mail: Tested by [%ANCHOR: test_backup_orchestration]
    ...
    def _trigger_pgbackrest_backup(self):
        try:
            ...
            res = subprocess.run(cmd, capture_output=True, text=True, timeout=3600, shell=False)
            if res.returncode != 0:
                self._report_backup_failure(f"pgBackRest backup failed: {res.stderr}")
            else:
                self.message_post(body=_("pgBackRest backup completed successfully.")) # audit-ignore-mail: Tested by [%ANCHOR: test_backup_orchestration]
                self.action_sync_snapshots()
        except Exception as e:
    ...
    def _apply_kopia_policies(self):
        try:
            ...
            cmd.extend([f'--keep-latest={self.keep_daily}', f'--keep-daily={self.keep_daily}', f'--keep-weekly={self.keep_weekly}', f'--keep-monthly={self.keep_monthly}'])
            if self.exclude_patterns:
                for line in self.exclude_patterns.splitlines():
                    if line.strip():
                        cmd.append(f'--add-ignore={line.strip()}')
            res = subprocess.run(cmd, capture_output=True, text=True, timeout=30, env=env_vars, shell=False)
            if res.returncode != 0:
                self._report_backup_failure(f"Kopia policy set failed: {res.stderr}")
            else:
                self.message_post(body=_("Kopia policies applied successfully.")) # audit-ignore-mail: Tested by [%ANCHOR: test_backup_orchestration]
        except Exception as e:
    ...
    def _report_backup_failure(self, message):
        ...
        if 'pager.incident' in self.env:
            try:
                ...
                pass
        self.message_post(body=message) # audit-ignore-mail: Tested by [%ANCHOR: backup_pager_synergy]
    ...
    def get_board_data(self):
        ...
        for c in configs:
            snap = self.env['backup.snapshot'].search_read([('config_id', '=', c['id'])], ['snapshot_id', 'start_time', 'size_bytes', 'status'], order='start_time desc', limit=1)
            if snap:
                c['latest_snapshot'] = snap[0]
                delta = (now - snap[0]['start_time']).total_seconds() if snap[0]['start_time'] else 999999
    ...
    def _sync_pgbackrest(self):
        try:
            exe = self._get_executable('pgbackrest')
            res = subprocess.run([exe, 'info', f'--stanza={self.target_path}', '--output=json'], capture_output=True, text=True, timeout=30, shell=False)
            if res.returncode != 0:
            ...
            for snap in data:
                ...
                if sid and sid not in existing_map:
                    creates.append({
                        'config_id': self.id,
                        'snapshot_id': sid,
                        'start_time': snap.get('startTime', '')[:19].replace('T', ' '),
            ...
            for stanza in data:
                for snap in stanza.get('backup', []):
                    ...
                    if sid and sid not in existing_map:
                        ...
                        dt = datetime.datetime.utcfromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S') if ts else False
                        creates.append({
                            'config_id': self.id,
                            'snapshot_id': sid,
                            'start_time': dt,
            ...
            if self.minimum_size_mb > 0:
                for c in creates:
                    ...
                    if snap_mb < self.minimum_size_mb:
                        self._report_backup_failure(f"Snapshot Anomaly: Snapshot {c.get('snapshot_id')} is {snap_mb:.2f} MB, below the {self.minimum_size_mb} MB minimum.")
    ...
    def _execute_restore_drill(self):
        try:
            res = subprocess.run([self.restore_drill_script], capture_output=True, text=True, timeout=7200, shell=False)
            if res.returncode != 0:
                self._report_backup_failure(f"Automated Restore Drill FAILED: {res.stderr}")
            else:
                self.last_drill_time = fields.Datetime.now()
                self.message_post(body=_("Automated Restore Drill completed successfully.")) # audit-ignore-mail: Tested by [%ANCHOR: test_backup_cron]
        except Exception as e:

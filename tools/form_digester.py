...
def get_field_label(page, widget):
    ...
    if widget.field_type in [fitz.PDF_WIDGET_TYPE_CHECKBOX, fitz.PDF_WIDGET_TYPE_RADIOBUTTON]:
        ...
        search_rect = fitz.Rect(rect.x0 - 50, rect.y0 - 20, max(rect.x1, rect.x0 + 150), rect.y0 + 15)
    ...
    clean_text = re.sub(r'\s+', '_', clean_text).lower().strip('_') # Replace spaces with underscores
...
def convert_pdf_to_svg_html(pdf_path, output_dir):
    ...
    for page_num in range(len(doc)):
        ...
        html_content.append(f'  <div class="page-container" style="width: {page_width}px; height: {page_height}px;">')
        ...
        for widget in page.widgets():
            ...
            pdf_internal_name = widget.field_name or f"unnamed_pdf_field_{page_num}_{hash(rect)}"
            ...
            if widget.field_type == fitz.PDF_WIDGET_TYPE_CHECKBOX:
                checked = "checked" if str(f_value).lower() in ["yes", "on", "/yes", "/on", "true"] else ""
                html_content.append(f'    <input type="checkbox" class="checkbox-field" name="{html_name}" style="{style}" {checked}>')
                
            elif widget.field_type == fitz.PDF_WIDGET_TYPE_RADIOBUTTON:
                html_content.append(f'    <input type="radio" class="checkbox-field" name="{html_name}" value="{f_value}" style="{style}">')
                ...
                if rect.height > 25:
                    html_content.append(f'    <textarea class="form-field" name="{html_name}" style="{style}">{f_value}</textarea>')
                else:
                    html_content.append(f'    <input type="text" class="form-field" name="{html_name}" value="{f_value}" style="{style}">')

        html_content.append('  </div>')

    html_content.append("  <button type='submit' style='padding: 10px 20px; font-size: 16px; margin-top: 20px; cursor: pointer; background-color: #0056b3; color: white; border: none; border-radius: 4px;'>Submit ICS Form</button>")

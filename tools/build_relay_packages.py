...
RELAY_SOURCE_PATH = os.path.join(BASE_DIR, 'daemons', 'hams_local_relay', 'hams_local_relay.py')
...
def fetch_latest_hamlib_windows():
    ...
    try:
        req = urllib.request.Request(api_url, headers={'User-Agent': 'Hams-Build-Script'})
        with urllib.request.urlopen(req) as response:
            data = json.loads(response.read().decode('utf-8'))
        for asset in data.get('assets', []):
            if asset['name'].startswith('hamlib-w64-') and asset['name'].endswith('.zip'):

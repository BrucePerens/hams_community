...
5.  **LARGE FILE PROHIBITION (CRITICAL):** The Service Worker includes a hardcoded 5MB safety valve. If you place a file larger than 5MB in a `static/` directory, the Service Worker will deliver it to the browser but will intentionally **refuse to cache it** to protect the browser's storage quota. Heavy media or user-generated data must be served via non-cached dynamic routes like `/web/image` or `/web/content`.
